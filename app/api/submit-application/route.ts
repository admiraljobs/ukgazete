...
/**
 * Upload a base64 image to Firebase Storage (private) and return a
 * 7-day signed URL suitable for including in the admin notification email.
 * The storage path is stored in Firestore for later retrieval.
 */
...
export async function POST(request: Request) {
  try {
    ...
    // ── Upload photos (stored privately; signed URLs for admin email) ─────
    const photoSignedUrls: Record<string, string> = {};
    const photoStoragePaths: Record<string, string> = {};
    ...
    if (formData.selfiePhoto?.startsWith('data:')) {
      try {
        ...
        photoSignedUrls.selfie = result.signedUrl;
        photoStoragePaths.selfie = result.storagePath;
      } catch (uploadErr) {
    ...
    if (formData.passportPhoto?.startsWith('data:')) {
      try {
        ...
        photoSignedUrls.passport = result.signedUrl;
        photoStoragePaths.passport = result.storagePath;
      } catch (uploadErr) {
    ...
    const applicationData = {
      ...
      selfiePhotoPath: photoStoragePaths.selfie || null,
      passportPhotoPath: photoStoragePaths.passport || null,
