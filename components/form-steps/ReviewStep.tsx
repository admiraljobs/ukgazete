    ...
    <div className="p-5 rounded-xl bg-surface-elevated border border-brand-royal">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-brand-dark font-medium">{title}</h3>
        </div>
        <button
          type="button"
          onClick={onEdit}
...
function DataRow({ label, value }: { label: string; value?: string | null }) {
  return (
    <div className="flex justify-between py-1.5">
      ...
      <span className="text-brand-dark font-medium text-sm">{value || '—'}</span>
    </div>
  );
}
  ...
  if (showPayment && clientSecret) {
    return (
      <div>
        ...
        {/* Payment Summary */}
        <div className="p-5 rounded-xl bg-surface-elevated border border-brand-royal mb-6">
          <div className="space-y-2 text-sm">
            ...
            <div className="pt-2 border-t border-brand-royal">
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="space-y-4 mb-8">
        ...
        >
          <DataRow label={t('review.labels.issuingCountry')} value={getCountryName(formData.passportCountry)} />
          <DataRow label={t('review.labels.passportNumber')} value={formData.passportNumber} />
          <DataRow label={t('review.labels.issueDate')} value={formData.issueDate} />
          <DataRow label={t('review.labels.expiryDate')} value={formData.expiryDate} />
          <DataRow label={t('review.labels.issuingAuthority')} value={formData.issuingAuthority} />
        </ReviewSection>
        ...
        <ReviewSection
          ...
          editLabel={t('review.edit')}
        >
          <DataRow label={t('review.labels.fullName')} value={`${formData.firstName} ${formData.lastName}`} />
          <DataRow label={t('review.labels.dateOfBirth')} value={formData.dateOfBirth} />
          <DataRow label={t('review.labels.gender')} value={formData.gender} />
          <DataRow label={t('review.labels.nationality')} value={getCountryName(formData.nationality)} />
          <DataRow label={t('review.labels.birthCountry')} value={getCountryName(formData.birthCountry)} />
        </ReviewSection>
        ...
        <ReviewSection
          ...
          editLabel={t('review.edit')}
        >
          <DataRow label={t('review.labels.email')} value={formData.email} />
          <DataRow label={t('review.labels.phone')} value={formData.phone} />
        </ReviewSection>
        ...
        >
          {formData.selfiePhoto ? (
            <div className="flex items-center gap-2 text-green-600">
              ...
              <span>{t('review.labels.photoUploaded')}</span>
            </div>
          ) : (
            <span className="text-red-500">{t('review.labels.noPhoto')}</span>
          )}
        </ReviewSection>
        ...
        >
          <DataRow
            label={t('review.labels.criminalConvictions')}
            value={formData.criminalConvictions === 'yes' ? t('common.yes') : t('common.no')}
          />
          <DataRow
            label={t('review.labels.immigrationBreaches')}
            value={formData.immigrationBreaches === 'yes' ? t('common.yes') : t('common.no')}
          />
          <DataRow
            label={t('review.labels.previousRefusals')}
            value={formData.previousRefusals === 'yes' ? t('common.yes') : t('common.no')}
          />
          <DataRow
            label={t('review.labels.terrorismInvolvement')}
            value={formData.terrorismInvolvement === 'yes' ? t('common.yes') : t('common.no')}
          />
        </ReviewSection>
        ...
        >
          ...
          />
          <DataRow label={t('review.labels.city')} value={formData.city} />
          <DataRow label={t('review.labels.postcode')} value={formData.postcode} />
          <DataRow label={t('review.labels.residenceCountry')} value={getCountryName(formData.country)} />
        </ReviewSection>
        ...
        >
          {formData.emergencyName ? (
            <>
              <DataRow label={t('review.labels.emergencyName')} value={formData.emergencyName} />
              <DataRow label={t('review.labels.relationship')} value={formData.emergencyRelationship} />
              <DataRow label={t('review.labels.emergencyPhone')} value={formData.emergencyPhone} />
            </>
          ) : (
            <span className="text-brand-muted">{t('review.notProvided')}</span>
          )}
        </ReviewSection>
      </div>
      ...
      {/* Payment Summary */}
      <div className="p-6 rounded-xl bg-surface-elevated border border-brand-royal mb-8">
        <h3 className="text-lg font-medium text-brand-dark mb-4">{t('payment.title')}</h3>
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className="pt-3 border-t border-brand-royal">
        ...
        <div className="flex items-center gap-3 p-3 rounded-lg bg-surface-card border border-brand-royal">
          <CreditCard className="w-5 h-5 text-brand-muted" />
          <span className="text-brand-muted text-sm">{t('payment.methods')}</span>
        </div>

        <div className="flex items-center gap-2 mt-4 text-brand-muted text-xs">
